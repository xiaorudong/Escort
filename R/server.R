server <- function(input, output, session) {
  ...
  output$step1_dc <- renderText({
    if (!mydf$from & is.null(step1_test2())) {
      return(NULL)
    }
  ...
  output$downloadStep1de <- downloadHandler(
    filename = function() {
      paste("step_1_DE", Sys.Date(), ".rds", sep = "")
    },
    content = function(file) {
      req(step1_de())
      saveRDS(step1_de(), file)
    }
  )
  ...
  generate_embedding_plot <- function(embedding_name, embedding_object) {
    ...
    segments(
      x0 = embedding_object$fitLine$x0,
  ...
  step2_test1 <- reactive({
    ...
    if (is.null(step1_res()) && is.null(mydf$result)) {
      return(NULL)
    }
    DRLvsC <- list()
